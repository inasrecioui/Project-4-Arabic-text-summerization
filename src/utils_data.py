...
class GPT2Dataset(Dataset):
    def __init__(self, df):
        self.encodings = df['encodings'].apply(ast.literal_eval).to_list()
    ...
    def __getitem__(self, idx):
        text_encoding = self.encodings[idx]
        text = torch.tensor(text_encoding['input_ids']).to(torch.long)
        attn_mask = torch.tensor(text_encoding['attention_mask']).to(torch.long)
        s_idx = self.sum_idx[idx] + 2
